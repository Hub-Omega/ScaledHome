...
class Sun(sunMotor):
    def __init__(self,name,pin1,pin2,enable,duty):
        ...
        self.p = GPIO.PWM(self.enable, 1000)
...
class ControlTower():
    ...
    def convertToSimulationTime(self, time):
        '''
        ...
        '''
        convert = time.values.tolist()
        for i in range(len(convert)):
            militaryTime = convert[i].split(':')
            parse = [int(digit) for digit in militaryTime]
            convert[i] = (parse[0]*3600 + parse[1]*60) // 45
        return convert
    
    def parseCSV(self, csv: str, object):
        
        startTimes = pd.read_csv(csv, usecols = ['Objects','on_time'])
        endTimes = pd.read_csv(csv, usecols = ['Objects','off_time'])
        #startTimes.join(endTimes.set_index(['Objects','off_time']), on = ['Objects','on_time'], how = 'left')
        ...
        events['time'] = self.convertToSimulationTime(events['time'])
        print(events['time'])
##        time = pd.DataFrame(data = None)
##        print(events)
##        for row in events:
##            print(row)
##            print(self.convertToSimulationTime(events.iat[row, 1]))
        
            # time.append(self.convertToSimulationTime(event[1]))
        ...
        events['Objects'] = object_list
        print('event', events['Objects'])
        return events
##    commandList = {}
##    with open(filename, "r") as simFile, open(outFile, "w") as outFile:
##        csvFile = csv.reader(simFile)
    def simulation(self, object):
        sim = self.parseCSV('Scenario.csv', object)
        counter = 0
        startTime = time.time()
        
        while(counter < len(sim.index)):
            timestamp = time.time()
            delta = int(timestamp - startTime)
##            print(delta)
##            print(sim['time'][counter])
            if (delta == sim['time'][counter]):
                if (sim.iat[counter, 0].getState()):
                    sim.iat[counter, 0].turnOff()
                    sim.iat[counter, 0].state = 0
                else:
                    sim.iat[counter, 0].turnOn()
                    sim.iat[counter, 0].state = 1
                counter += 1
